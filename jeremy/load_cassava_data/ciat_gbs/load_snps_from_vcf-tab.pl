...
IMPORTANT
The input comes from vcf-to-tab
need to sed 's/\t/_/' the input file first
...
our ($opt_H, $opt_D, $opt_i, $opt_t, $opt_p, $opt_y, $opt_g, $opt_L);

getopts('H:i:tD:p:y:g:L:');
...
if (!$opt_H || !$opt_D || !$opt_i || !$opt_g || !$opt_L) {
  pod2usage(-verbose => 2, -message => "Must provide options -H (hostname), -D (database name), -i (input file) , -g (populations name for associating accessions in your SNP file), -L (location) \n");
}
