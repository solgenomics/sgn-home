...
the agpdir should contain a file per chromosome, with the .agp extension.

The local2acc_id should contain two tab separated columns with the local id and the corresponding Genbank accession.

=head2 TPF SPECS

Column definitions for sequence lines: 

Column 1: accession number. This must be a valid accession assigned by the International DNA Sequence Database (GenBank/EMBL/DDBJ). No version number should be used. It is assumed that the current version of the accession is what is intended. OPTIONAL,use "?" if unknown. 

Column 2: clone name. If the sequence is defined by a clone, then the clone name should be specified here. Ideally, standard Clone Registry nomenclature will be used. If a sequence is derived from more than one clone, the clone name can be given as "MULTIPLE". OPTIONAL, use "?" if unknown. 

Column 3: local contig identifier. Name given to the scaffold/contig generated by assembling the listed sequences. REQUIRED.
*	NOTE: Use of "." (the "dot" character) is not allowed in contig identifiers.
*	NOTE: A local contig identifier may only be used once per assembly.

Column 4: contained status. This line is only used to specify clones that are known to be contained within another clone on the TPF. The only values allowed are "CONTAINED" and "CONTAINED_TURNOUT". REQUIRED for contained clones, otherwise OPTIONAL.
